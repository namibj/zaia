//! Nodes, tokens, elements and their references which are guaranteed to belong
//! to trees with associated [`Resolver`]s(lasso::Resolver).
//!
//! This means they can implement `Debug` and `Display` and be (de-)serializable
//! by default.
...
/// Syntax tree node that is guaranteed to belong to a tree that contains an
/// associated [`Resolver`](lasso::Resolver).
/// # See also
/// [`SyntaxNode`]
...
impl<L: Language, D> ResolvedNode<L, D> {
    /// # Safety:
    /// `syntax` must belong to a tree that contains an associated inline
    /// resolver.
...
/// Syntax tree token that is guaranteed to belong to a tree that contains an
/// associated [`Resolver`](lasso::Resolver).
/// # See also
/// [`SyntaxToken`]]
#[repr(transparent)]
...
/// An element of the tree that is guaranteed to belong to a tree that contains
/// an associated [`Resolver`](lasso::Resolver), can be either a node or a
/// token. # See also
...
/// A reference to an element of the tree that is guaranteed to belong to a tree
/// that contains an associated [`Resolver`](lasso::Resolver), can be either a
/// reference to a node or one to a token. # See also
/// [`SyntaxElementRef`]
pub type ResolvedElementRef<'a, L, D = ()> =
    NodeOrToken<&'a ResolvedNode<L, D>, &'a ResolvedToken<L, D>>;
...
impl<L: Language, D> ResolvedNode<L, D> {
    /// Uses the resolver associated with this tree to return an efficient
    /// representation of all source text covered by this node, i.e. the
    /// combined text of all token leafs of the subtree originating in this
    /// node.
...
impl<L: Language, D> ResolvedToken<L, D> {
    /// Uses the resolver associated with this tree to return the source text of
    /// this token.
...
impl<L: Language, D> ResolvedNode<L, D> {
    ...
    ///
    /// This method mostly exists to allow the convenience of being agnostic
    /// over [`SyntaxNode`] vs [`ResolvedNode`].
    ...
    /// [`children_with_tokens`](ResolvedNode::children_with_tokens).
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`first_child_or_token`](ResolvedNode::first_child_or_token).
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`last_child_or_token`](ResolvedNode::last_child_or_token).
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`next_child_or_token_after`](ResolvedNode::next_child_or_token_after).
    ...
    pub fn next_child_or_token_after(
        ...
        n: usize,
        offset: TextSize,
    ) -> Option<ResolvedElementRef<'_, L, D>> {
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`prev_child_or_token_before`](ResolvedNode::
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`next_sibling_or_token`](ResolvedNode::next_sibling_or_token).
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`prev_sibling_or_token`](ResolvedNode::prev_sibling_or_token).
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`siblings_with_tokens`](ResolvedNode::siblings_with_tokens).
    ...
    pub fn siblings_with_tokens(
        &self,
        direction: Direction,
    ) -> impl Iterator<Item = ResolvedElementRef<'_, L, D>> {
    ...
    ///
    /// If you want to also consider leafs, see
    /// [`descendants_with_tokens`](ResolvedNode::descendants_with_tokens).
    ...
    pub fn preorder_with_tokens(
        &self,
    ) -> impl Iterator<Item = WalkEvent<ResolvedElementRef<'_, L, D>>> {
        self.syntax
...
impl<L: Language, D> ResolvedToken<L, D> {
    ...
    ///
    /// This method mostly exists to allow the convenience of being agnostic
    /// over [`SyntaxToken`] vs [`ResolvedToken`].
